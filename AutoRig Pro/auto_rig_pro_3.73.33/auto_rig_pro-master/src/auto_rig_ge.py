...
class ARP_OT_GE_actions_linker(Operator):
    ...
    def draw(self, context):
        ...
        if len(scn.arp_actions_links):
            ...
            #row.prop(act_link, 'action_a', text='')
            #row.prop(act_link, 'action_b', text='')
    ...
    def execute(self, context):
        ...
        try:
            ...
            if scn.arp_bake_anim and scn.arp_bake_type == 'ACTIONS' and scn.arp_export_separate_fbx:
                if scn.arp_export_separate_fbx_list and scn.arp_export_use_actlist:
                    
                    # collect exportable actions for each list
                    export_ids = {}
                    
                    for act_list in scn.arp_export_actlist:
                        if not act_list.exportable:
                            continue
                        
                        actions_list = []
                        
                        for act_item in act_list.actions:
                            if act_item.action == None:# the action was removed by the user
                                continue
                                
                            action_baked_name = act_item.action.name.replace('_%temp', '')                            
                            act = bpy.data.actions.get(action_baked_name)
                            if act:
                                if not is_action_exportable(act):
                                    continue
                            
                                actions_list.append(act.name)
                            
                        export_ids[act_list.name] = actions_list
            ...
            for export_id in export_ids:
                ...
                ext = '.fbx'
                if self.export_format == 'GLTF':
                    if scn.arp_ge_gltf_format == 'GLTF_SEPARATE':
                        ext = '.gltf'
                    elif scn.arp_ge_gltf_format == 'GLB':
                        ext = '.glb'
                ...
                if export_id != 'all_actions':
                    if scn.arp_export_file_separator == 'NONE':
                        ...
                        def_fp = self.filepath[:-4]+scn.arp_export_file_separator+export_id+ext
                        
                def_fps.append(def_fp)# store the exported files paths for info box
...
def create_copies(self):
    ...
    mods_types = ['NORMAL_EDIT', 'WEIGHTED_NORMAL', 'ARRAY', 'BEVEL', 'BOOLEAN', 'BUILD', 'DECIMATE', 'EDGE_SPLIT', 'NODES', 'MASK',
    ...
    if scn.arp_apply_mods:
        ...
        for obj_name in self.char_objects + self.spline_objects:
            ...
            apply_mods = True if self.export_format == 'GLTF' else False
            has_sk = False
            ...
            if apply_mods:
                ...
                if has_sk:
                    ...
                    for i in reversed(range(len(obj_exp.data.shape_keys.key_blocks))):             
                        obj_exp.shape_key_remove(obj_exp.data.shape_keys.key_blocks[i])# faster than operator
...
def _set_mannequin_orientations(self):
    ...
    # ensure all deforming bones are in layer 17/Deform collection
    for ebone in rig_humanoid.data.edit_bones:
        if ebone.use_deform:
            if bpy.app.version >= (4,0,0):
...
def _bake_all(armature_name, baked_armature_name, self):
    ...
    if scn.arp_bake_type == "NLA":
        ...
        if self.quick_rig_control_armature:
            if get_object(self.quick_rig_control_armature).animation_data:
                if get_object(self.quick_rig_control_armature).animation_data.action:
                    original_action_ctrl_rig = get_object(self.quick_rig_control_armature).animation_data.action
        ...
        for action in actions:
            ...
            if bake_this_action:
                #print("  Baking action because bake_this_action is TRUE:", action.name)
                ...
                if self.quick_rig_control_armature:
                    get_object(self.quick_rig_control_armature).animation_data.action = action
        ...
        if self.quick_rig_control_armature:
            get_object(self.quick_rig_control_armature).animation_data.action = original_action_ctrl_rig
...
def _set_mped_rig(armature_name, armature_add_name, manual_set_debug, self):
    ...
    def find_matching_parent(par_name=None, bone_name=None):
        ...
        def fallback_parent(par_name):
            print('  Error with '+bone_name+', could not find the associated parent bone in the M-Ped armature: '+par_name+'\nAssign to "root.x" bone instead.')
            par_name = "root.x"
...
def export_panel_draw(self, context):
    ...
    def anim_export_ui():
        ...
        if scn.arp_bake_type == 'ACTIONS':
            ...
            col = col.column()
            col.enabled = scn.arp_export_use_actlist and scn.arp_export_separate_fbx
            col.prop(scn, 'arp_export_separate_fbx_list' , text='One File per Actions List')
        ...
        if scn.arp_export_use_actlist:
            ...
            col.prop(scn, "arp_bake_only_active")
...
def register():
    ...
    bpy.types.Scene.arp_global_scale = FloatProperty(name="Global Scale", default = 1.0, description="Global scale applied", options={'HIDDEN'})
    ...
    bpy.types.Scene.arp_init_fbx_rot = BoolProperty(name='Initialize Fbx Armature Rotation', description='Export armature with rotations values set to 0', default = False, options={'HIDDEN'})#, update=update_init_rot_btn)
    ...
    bpy.types.Scene.arp_export_separate_fbx_list = BoolProperty(default=False, description='Export one file per list of actions as defined in the Action Manager', options={'HIDDEN'})
    ...
    bpy.types.Scene.arp_ge_add_dummy_mesh = BoolProperty(default=True, name="Add Dummy Mesh", description='Add a blank mesh object with no vertices when the skeleton only is exported, to prevent import issues.\nAutomatically enabled when exporting to UE, since skeleton without meshes cannot be imported in UE', options={'HIDDEN'})
    bpy.types.Scene.arp_ge_force_rest_pose_export = BoolProperty(default=False, name="Force Rest Pose Export", description='Export the bind/rest pose even if only the skeleton is exported, without meshes', options={'HIDDEN'})
